...
export default function LoginPage() {
  ...
  async function handleLogin() {
    try {
      ...
      const userDoc = await axios.post(`${baseAPIPath}/login`, {
        email: email,
        password: password,
      });
      setUser(userDoc.data);
      navigate("/", {
        state: { email: email, password: password, role: userDoc.data?.role },
      });
    } catch (e: any) {
    ...
    <div className="mt-4 grow flex items-center justify-center min-h-screen bg-gradient-to-r from-purple-500 to-indigo-600">
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
        ...
        <form className="space-y-4">
          ...
          <button
            type="button"
            className="bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-full w-full font-semibold transition duration-200 transform hover:scale-105"
            onClick={() => navigate("/")}
          >
            Cancel
          </button>
